...
def createModel():
    ...
    model.add(tf.keras.layers.Dense(200, activation=actF))
    for i in range(N):
        model.add(tf.keras.layers.Dense(100, activation=actF))
    model.add(tf.keras.layers.Dense(100, activation=actF))
    model.add(tf.keras.layers.Dense(1, activation=actF))
...
for optimizer in opt_list:
  ...
  plt.plot(N_list, acc_list,label=optimizer + ' - train', marker='o')
  plt.plot(N_list, acc_train_list,label=optimizer + ' - test', ls='dotted', color= 'C' + str(opt_list.index(optimizer)))
  print('*************************************************************************************************************')
  print(optimizer)

plt.legend(frameon = True, fancybox = True, facecolor='white', framealpha=1)
plt.title('NN za različno število skritih plasti')
#plt.xscale('log')
plt.xlabel('število skritih plasti')
plt.ylabel('MSE')
plt.show()
