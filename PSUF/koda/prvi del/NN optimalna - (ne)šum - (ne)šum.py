...
IntensityNoise = np.load(f'Podatki\\DataK\\intensity{lbd}noise100.npy')[0:N]
...
def accept(event):
    if event.key == "enter":
        ...
        for i in Y_pred_ind:
            plots = axx.plot(time, X_test[i], alpha=0.05, c='0')
        axx.plot(time,np.mean(X_test[np.array(Y_pred_ind)], axis=0), c='C' +str(step))
        axx.set_xlabel('$t$ [s]')
