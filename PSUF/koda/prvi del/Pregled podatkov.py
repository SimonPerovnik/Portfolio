...
matplotlib.rcParams["savefig.directory"] = "E:\\Faks\\5.letnik\\Praktikum strojnega učenja\\2. naloga\\slike"
...
selection = [150, 300, 43, 90, 555]    # Some hand-picked interesting cases

# Plot some intensities

plt.figure(figsize=(7,3))
for i in selection:
    plt.plot(time[:Nt//4], Intensity[i][:Nt//4], label=f'$K={Kvalues[i]/1e-13:.2f}$ pN')
plt.xlabel('$t$ [s]')
...
for i in range(4):
    ...
    for theta in theta_t:
        ...
        j += 1
plt.suptitle('Relaksacija direktorskega polja')
