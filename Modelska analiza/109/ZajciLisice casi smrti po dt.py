...
for dt in dt_all:
    ...
    plt.plot( bins[1:],hist,
              label =  '$dt =$ ' + str(dt) + ', $t_{mode} = $' + str(round(bins[np.argmax(hist)],1)) + ', $t_{mean} = $' + str(round(t_mean,1)),
              alpha = 0.6)
plt.legend(frameon = True, fancybox = True, facecolor='white', framealpha=1)
plt.xlabel('$t_{death}$')
plt.ylabel('$N$')
plt.title('Časi izumrtja - $Z_0 = $' + str(Z0) + ', $L_0 = $' + str(L0)  + ', $\u03B1/\u03B2 = $' + str(ratio) + ', število ponovitev $= $' + str(iterations))
