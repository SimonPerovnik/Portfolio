...
for ratio in ratio_all:
    ...
    hist = np.histogram(t_hist, bins = numBin, range = (0,tBin_max))
    t_death_all[0] = 0 
    plt.plot( bins,t_death_all,
              label =  '$\u03B1/\u03B2 = $' + str(ratio) + ', $t_{mode} = $' + str(round(bins[np.argmax(t_death_all)],1)) + ', $t_{mean} = $' + str(round(t_mean,1)),
              alpha = 0.6)
