...
def startEquidistant():
    #Random starting points
    start = [0,0,0,m.pi] #prvi delec fiksiramo na 0,0
    for i in range(N-2):
...
for i in range(len(start_method)):
    ...
    for N in N_list:
        ...
        iterations_list.append(numIter/(j+1))
        energy_list.append(energy/(j+1))
        print(N)

    ax1.plot(N_list, iterations_list, label=start_methodStr[i], marker= 'o')
    ax2.plot(N_list, energy_list, label=start_methodStr[i], marker= 'o')
plt.legend(frameon = True, fancybox = True, facecolor='white', framealpha=1)
fig.set_figheight(5)
fig.set_figwidth(10)
ax1.set_xlabel('$N$')
ax1.set_title('Število iteracij, potrebnih za minimizacijo')
ax2.set_xlabel('$N$')
ax2.set_ylabel('$E$', rotation= 0)
ax2.set_title('Končna energija minimizacije')
plt.show()
