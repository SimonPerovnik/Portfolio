...
for k in range(len(start_method)):


    start = start_method[k]
    data = []
    ...
    z3 = C*np.cos(v)
    ax.plot_surface(x3, y3, z3, alpha = 0.1, cmap = 'twilight')
    #ax.view_init(90, 0)
    ...
    matplotlib.style.use('seaborn')
    if PlottingPhase:
        data = [rotatePoints(i) for i in data]
        p = len(data)
        data = np.transpose(data)
        
        for i in range(0,2*N,2):
            xx,yy,zz = [],[],[]
            for j in range(0,p, 50):
                if (A!=1 or B!=1 or C!=1):  #na elipsi ne fiksiramo delca na 0,0
                    rotatedResult = result.x
                xx.append(polarToCartesian(data[i][j], data[i+1][j])[0])
                yy.append(polarToCartesian(data[i][j], data[i+1][j])[1])
                zz.append(polarToCartesian(data[i][j], data[i+1][j])[2])
            t = np.arange(len(xx))
            ax.scatter(xx,yy,zz, s=50, c=t, cmap='viridis')
            ax.plot(xx,yy,zz, lw=2, ls='dotted')
        fig.set_figheight(5)
        fig.set_figwidth(5)
        plt.tight_layout()
        plt.title(start_methodStr[k] +' - Število iteracij: ' + str(result.nit))
        plt.show()
        ...
        #data = np.transpose(data)
        for i in range(0,2*N,2):
            plt.scatter(data[i], data[i+1],s=20, c=t, cmap='viridis')
        plt.xlabel('$\phi$')
        plt.ylabel('$\\theta$')
        plt.title('Konvergenca točk proti minimumu med simulacijo')
        plt.show()
