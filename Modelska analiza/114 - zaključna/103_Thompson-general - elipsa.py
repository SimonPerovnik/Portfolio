...
matplotlib.rcParams["savefig.directory"] = r"E:\Faks\4.letnik\Modelska_analiza_I\114 - zaključna\slike\elipsa"

Plotting = 1
PlottingVolume = 0
PlottingPhase = 0
N = 5 #Number of charged particles
...
# method_of_choice = 'COBYLA'
# method_of_choice = 'CG'
# method_of_choice = 'trust-ncg'
...
def elEnergy(*arg):
    ...
    for i in range(0,len(combs_flat),4):
        ...
        r1 = np.array(polarToCartesian(combs_flat[i], combs_flat[i+1]))
        r2 = np.array(polarToCartesian(combs_flat[i+2], combs_flat[i+3]))
        phi = np.arccos(np.dot(r1,r2)/(np.linalg.norm(r1) * np.linalg.norm(r2)))
...
#start = [0,0,0,np.pi/2,1.5,3/2,3,3/2,4.5,3/2,0,3]
#Data points memory
data = []
...
z3 = C*np.cos(v)
ax.plot_surface(x3, y3, z3, alpha = 0.2, cmap= 'twilight')
#ax.view_init(90, 0)
