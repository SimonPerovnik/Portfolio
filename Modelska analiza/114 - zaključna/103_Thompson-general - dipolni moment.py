...
for N in N_list:
    ...
    r_all = []
    for k in range(3):    
        result = minimize(elEnergy, start, method= method_of_choice, options={'fatol': 0.000000001, 'adaptive': True, 'disp': False})
        ...
        r_all.append(np.linalg.norm(sum(xx) + sum(yy) + sum(zz)))
    r.append(min(r_all))
    print(N)

plt.scatter(N_list, r, label='nov model', marker= 'o')
plt.xlabel('$N$')
plt.ylabel('$|\\vec{r}_i|$', rotation=0)
#plt.yscale('log')
plt.title('Velikost dipolnega momenta')
plt.show()
