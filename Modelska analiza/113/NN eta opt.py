...
for optimizer in optimizer_list:
  ...
  for eta in eta_list:
    ...
    if optimizer == 'nadam':
      optimizerM = tf.keras.optimizers.Nadam(learning_rate=eta)    
  ...
  plt.plot(eta_list, acc_list,label=optimizer + ' - train', marker='o')
  plt.plot(eta_list, acc_train_list,label=optimizer + ' - test', ls='dotted', color= 'C' + str(optimizer_list.index(optimizer)))
  print('*************************************************************************************************************')
  print(optimizer)

plt.legend(frameon = True, fancybox = True, facecolor='white', framealpha=1)
plt.title('Mreža (input -> $64$ -> $10$), activation fun.: swish, število epoh: 3')
plt.xscale('log')
plt.xlabel('$\eta$')
