...
list = []
file = open(r'E:\Faks\4.letnik\Modelska_analiza_I\111\kalman_cartesian_data.dat', "r")
list = file.readlines()
file.close()
...
for lines in list:
    ...
    vx.append(float(lines[3]))
    vy.append(float(lines[4]))
    ax.append(float(lines[5]))
    ay.append(float(lines[6][:-1]))
t = np.array(t)
x = np.array(x)
y = np.array(y)
vx = np.array(vx)
...
if PlotData:
    ...
    ax3.plot(t,ax, label = '$a_x$', lw=0.7)
    ax3.plot(t,ay, label = '$a_y$', lw=0.7)
...
def kalmanF():
    ...
    return F
def c(n):
    return np.array([0, 0, ax[n] * deltaT, ay[n] * deltaT ]).T
def kalmanQ():
    return np.diag([0, 0, sigma_a**2 * deltaT**2, sigma_a**2 * deltaT**2])
def kalmanR(n):
    return np.diag([sigma_xy**2, sigma_xy**2, sigma_v(n)**2, sigma_v(n)**2])
def sigma_v(n):
    return max(0.2778,np.sqrt(vx[n]**2 + vy[n]**2)*0.01 )
def kalmanH():
    return np.eye(4)
...
z = np.array([x,y,vx,vy]).T
x_0 = np.array([x[0], y[0],vx[0], vy[0]])
x_n = x_0
P_0 = np.eye(4)*5
P_n = P_0
F = kalmanF()
