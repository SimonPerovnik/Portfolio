...
T_list = np.linspace(1, 5, 25).tolist()
for H in [0,0.2,0.7]:
    ...
    axis[0,0].plot(T_list, magnetisation_list, label='$H = {}$'.format(H), marker='o')
    axis[0,0].set_title('$\langle M \\rangle (T)$')
    
    axis[0,1].plot(T_list, chi_list, label='$H = {}$'.format(H), marker='o')
    axis[0,1].set_title('$\langle \chi \\rangle (T)$')
    
    axis[1,0].plot(T_list, heatCap_list, label='$H = {}$'.format(H), marker='o')
    axis[1,0].set_title('$\langle C \\rangle (T)$')
    
    axis[1,1].plot(T_list, E_list, label='$H = {}$'.format(H), marker='o')
...
figure.suptitle('Isingov model : $J = {}$\n velikost mreže : ${} x {}$, število korakov: ${} \cdot 10^{}$'.format(J, N, N, int(str(steps)[0]),int( np.log10(steps))))
plt.show()