...
for alpha in alpha_list:
    ...
    for N in N_list:
        ...
        E_end.append(np.mean(E_steps[-200:])/N)
        E_all.append(E_steps)
    cmap = plt.get_cmap('viridis')
    plt.plot(N_list, E_end, label='$\\alpha$ = ' + str(alpha), c=cmap(alpha_list.index(alpha)/len(alpha_list)), marker= 'o', linewidth=0.9, markersize=4)

plt.title('Molekulska verižnica - končna energija v odvisnosti od števila členov\n' + '$T = {}'.format(T) +', h_{max}' +' = ${}, število korakov: {}'.format( h_max, steps))
plt.legend(frameon = True, fancybox = True, facecolor='white', framealpha=1, loc='best')
plt.ylabel('$\\frac{E_{end}(N)}{N}$', rotation=0)
#plt.xscale('log')
plt.xlabel('$N$')
plt.show()
