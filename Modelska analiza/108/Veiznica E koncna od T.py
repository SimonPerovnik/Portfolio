...
for alpha in alpha_list:
    ...
    for T in T_list:
        ...
        E_end.append(np.mean(E_steps[-200:]))
    ...
    plt.plot(T_list, E_end, label='$\\alpha$ = ' + str(alpha), c=cmap(alpha_list.index(alpha)/len(alpha_list)), marker= 'o', linewidth=0.9, markersize=4)
    print(alpha)

plt.title('Molekulska verižnica - končna energija\n' + '$h_{max}' +' = ${}, število korakov: {}'.format(h_max, steps))
plt.legend(frameon = True, fancybox = True, facecolor='white', framealpha=1)
plt.ylabel('$E_{end}(T)$', rotation=0)
plt.xscale('log')
plt.xlabel('$T$')
plt.show()
